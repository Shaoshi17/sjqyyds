---
created: 2023-12-11T10:33
updated: 2024-02-01T15:55
---
SQL
...
查其他库的数据库值
```
?id=-1* union select 1,2,group_concat(username,0x3a,password) from ctf.users --#
```
![[Pasted image 20240128110155.png]]
##### Mysql报错注入
...
--prefix=注入payload字符串前缀  有些奇葩闭合就可以绕过不用写tamper,这个适用于get和post
例如：--prefix="))))'"
...
###### 位移注入
使用另一个表联合进来，然后让前面为负，就会返回后面我们要看到的内容
![[Pasted image 20240130103226.png]]
![[Pasted image 20240130103417.png]]
注意：当前表要大于等于联合表。当前表就是article，联合表是admin.
###### 联合查询查询不存在数据会构造虚拟的数据
可以运用于各种地方。
联合查询本来就是临时新增了一列数据
[[GXYCTF 2019]BabySqli | NSSCTF](https://www.nssctf.cn/problem/1093)
![[Pasted image 20240128202547.png]]
...
```
sqlmap中无双写注入的脚本，所以我编写了一个tamper，可以根据测出的被ban的关键词进行修改,因为sqlmap喜欢用大写所以我们就用lower()给转换为小写：
```python
#!/usr/bin/env python
from lib.core.enums import PRIORITY
...
insert语句
post盲注之指定闭合
[[CISCN 2019华北Day2]Web1 | NSSCTF](https://www.nssctf.cn/problem/1)
什么^异或注入
## SQL-list做题
###### Level-12-16:POST盲注
